...
static void stack_event_handler(uint32_t event, void *event_param)
{
    ...
    case CY_BLE_EVT_GATTS_WRITE_REQ:
    {
        for(;;)
        {
            ...
            if(CY_BLE_RECORDING_TOGGLE_RECORDING_CHAR_HANDLE == write_req_param->handleValPair.attrHandle)
            {
                uint8_t recording =  write_req_param->handleValPair.value.val[0];
                ...
                {
                    printf("ERROR: Invalid Input. Please type 1 or 0. \r\n");
                }
                //write
                printf("[INFO] : GATT write RECORDING_TOGGLE characteristic with value: 0x%x\r\n", write_req_param->handleValPair.value.val[0]);
                printf("Recording: %d \r\n", RECORDING);
